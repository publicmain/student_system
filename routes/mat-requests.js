/**
 * routes/mat-requests.js — 中介材料收集系统 (Material Requests, UIF workflow, Companies/Contacts)
 * Split from routes/agent-portal.js
 */
const express = require('express');

module.exports = function({ db, uuidv4, audit, requireAuth, requireRole, upload, fileStorage, moveUploadedFile, sendMail, escHtml, brandedEmail, crypto, pdfGenerator, UPLOAD_DIR, fs, path, agentCallAttempts, AGENT_CALL_MAX, AGENT_CALL_WINDOW_MS }) {
  ...
  // Expose shared helpers for agent-portal.js to use
  router._matSendInviteEmail = _matSendInviteEmail;
  router._matCreateMagicLink = _matCreateMagicLink;
  router._matValidateToken = _matValidateToken;
  router._matAudit = _matAudit;
  router._agentRateLimit = _agentRateLimit;
