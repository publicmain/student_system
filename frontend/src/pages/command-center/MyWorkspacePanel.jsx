...
const sections = [
  ...
  { key: 'feedback', icon: MessageCircle, label: '待回复反馈', color: 'amber' },
]
        ...
        {sections.map(s => {
          ...
          return (
            ...
            >
              ...
              <div>
                <p className="text-lg font-bold text-ink-primary dark:text-slate-100">{count}</p>
                <p className="text-[10px] text-ink-tertiary dark:text-slate-400">{s.label}</p>
              </div>
            </button>
          )
        })}
...
function TaskList({ items }) {
  ...
  return (
    <div className="divide-y divide-surface-2 dark:divide-slate-700">
      {items.map(t => (
        <div key={t.id} className="px-4 py-3 flex items-center gap-3 hover:bg-surface-0/50 dark:hover:bg-slate-800/50">
          ...
          <div className="flex items-center gap-1.5">
            {t.category && <Badge color="default" className="text-[8px]">{t.category}</Badge>}
...
function ReviewList({ items }) {
  if (!items?.length) return <EmptyRow label="暂无待审文书" />
  return (
    <div className="divide-y divide-surface-2 dark:divide-slate-700">
      {items.map(r => (
        <div key={r.id} className="px-4 py-3 flex items-center gap-3 hover:bg-surface-0/50 dark:hover:bg-slate-800/50">
...
function FeedbackList({ items }) {
  ...
  return (
    <div className="divide-y divide-surface-2 dark:divide-slate-700">
      {items.map(f => (
        <div key={f.id} className="px-4 py-3 flex items-center gap-3 hover:bg-surface-0/50 dark:hover:bg-slate-800/50">
          ...
          <Badge color="amber" className="text-[8px]">待回复</Badge>
        </div>
      ))}
    </div>
  )
